;(function(module, Raphael, undefined) {
  ...
  CartesianCanvas.prototype.applyClip = function applyClip(element) {
  ...
  CartesianCanvas.prototype.convert_x = function convert_x(x) {
    ...
    // x = Math.min(Math.max(min_x, x), max_x);
  ...
  CartesianCanvas.prototype.convert_y = function convert_y(y) {
    ...
    // y = Math.min(Math.max(min_y, y), max_y);
  ...
  Chart.prototype.fadein = function(duration) {
    ...
    for ( var set in this.sets ) {
      if ( this.sets.hasOwnProperty(set) ) {
        ...
        var fill_opacity = 1;
        var stroke_opacity = 1;
        s.animate({'fill-opacity': fill_opacity, 'stroke-opacity': stroke_opacity, 'opacity': opacity}, duration, 'linear', function() {
          el.show();
        });
      }
    }
  ...
  BarChart.prototype.render = function render(canvas) {
    ...
    for ( box_i = 0; box_i < points.length; box_i++ ) {
      ...
      if ( x >= this.canvas.get_graph_left() &&
        ...
        y = Math.min(this.canvas.get_graph_bottom(), Math.max(this.canvas.get_graph_top(), y));
        original_box_index[box_points.length] = box_i;
    ...
    var y0 = Math.min(canvas.get_graph_bottom(), Math.max(canvas.get_graph_top(), canvas.convert_y(0) + 0.5));
    var scratch_set;
    for ( box_i = 0; box_i < box_points.length; box_i++ ) {
      ...
      if ( rect.height > 0 ) {
        ...
        should_add = this.get_box(rect, original_box_index[box_i], scratch_set);
        if ( ! should_add && should_add !== undefined ) {
          scratch_set.remove();
        }
        else {
